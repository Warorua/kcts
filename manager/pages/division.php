		...
		<div class="panel-body">
			...
			<table id="position" class="table table-bordered table-condensed">
				...
				<tbody>
					...
					$query =  mysqli_query($conn2, "SELECT * FROM project_division order by division");
					$i = 1;
					...
					?>
						<tr>
							...
							<td style="text-transform:capitalize" class="text-center"><?php
																						...
																						foreach ($projectTypes as $row1) {
																							if ($row1['id'] == $row['project_type']) {
																								echo $row1['title'];
																							}
							...
							<td style="text-transform:capitalize" class="text-center">
								<center><a onclick="update_div(<?php echo $id ?>)"><i class="fa fa-pencil"></i> edit</a></center>
							</td>
						</tr>
...
<div class="col-md-4">
	...
	<a class="col-sm-12 btn btn-md btn-info" id="add_div" onclick="add_form()">
		<center><i class="fa fa-plus"></i> Add</center>
	</a>
	<br>
	<div id="add_form">
		...
		jQuery("#pos_form").submit(function(e) {
			...
			$.ajax({
				...
				success: function(html) {
					...
					var delay = 1500;
					setTimeout(function() {
						window.location = 'index.php?page=division';
					}, delay);
