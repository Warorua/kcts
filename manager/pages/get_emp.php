<form method="POST" id="att_form" >
   ...
   if($_GET['tid'] == 'all'){ 
      $query=  mysqli_query($conn2, "SELECT *,CONCAT(last_name,', ',first_name,' ',midname) as name FROM employee  where io = '1' order by name ");
        
       }else{
        $query=  mysqli_query($conn2, "SELECT *,CONCAT(last_name,', ',first_name,' ',midname) as name FROM team_member natural join employee where tid = '".$_GET['tid']."' order by name ");
        $query4=  mysqli_query($conn2, "SELECT *,CONCAT(last_name,', ',first_name,' ',midname) as name FROM project_team left join employee on project_team.eid=employee.eid where tid = '".$_GET['tid']."' ");
        $row4 = mysqli_fetch_assoc($query4);
         $id4 =$row4['eid']; ?>
         <tr>
        <th><?php echo ucwords($row4['name'])." (Foreman)" ?></th>
        <?php $query5 = mysqli_query($conn2,"SELECT * FROM attendance where eid = '$id4' and date_today = '$d' ");
    ...
    ?>
      <tr>
        ...
        <?php $query2 = mysqli_query($conn2,"SELECT * FROM attendance where eid = '$id' and date_today = '$d'  ");
