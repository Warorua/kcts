<form method="POST" id="att_form" >
   ...
   if($_GET['tid'] == 'all'){ 
      $query=  mysqli_query($conn2, "SELECT *,CONCAT(lastname,', ',firstname,' ',midname) as name FROM employee  where io = '1' order by name ");
        
       }else{
        $query=  mysqli_query($conn2, "SELECT *,CONCAT(lastname,', ',firstname,' ',midname) as name FROM team_member natural join employee where tid = '".$_GET['tid']."' order by name ");
        $query4=  mysqli_query($conn2, "SELECT *,CONCAT(lastname,', ',firstname,' ',midname) as name FROM project_team left join employee on project_team.eid=employee.eid where tid = '".$_GET['tid']."' ");
