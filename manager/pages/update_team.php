...
$query = mysqli_query($conn2,"SELECT *,concat(lastname,', ',firstname,' ',midname) as name FROM project_team left join employee on project_team.eid = employee.eid where tid = '".$_GET['id']."' ");
...
<div class="panel panel-default">
	...
	<div class="col-md-4">
		<div class="form-group">
			...
			<option value="<?php echo $row['eid'] ?>"><?php echo ucwords($row['name']) ?></option>
				<?php
				$fquery=mysqli_query($conn2,"SELECT *,concat(lastname,', ',firstname,' ',midname) as name,employee.eid as id from employee where employee.eid NOT IN (SELECT  project_team.eid from project_team ) or eid not in (SELECT team_member.eid from team_member) ");
		...
		<div class="form-group">
			...
			<option></option>
				<?php
				$mquery=mysqli_query($conn2,"SELECT *,concat(lastname,', ',firstname,' ',midname) as name from employee where employee.eid NOT IN (SELECT  project_team.eid from project_team ) or eid not in (SELECT team_member.eid from team_member) ");
	...
	<div class="col-md-6">
		<table class="table table-bordered">
			...
			$query1 = mysqli_query($conn2,"SELECT *,concat(lastname,', ',firstname,' ',midname) as name FROM team_member left join employee on team_member.eid = employee.eid where tid = '".$_GET['id']."' ");
