...
<table id="teams" class="table table-bordered">
	...
	<tbody>
		...
		$query = mysqli_query($conn2,"SELECT *,Concat(firstname,', ',lastname,' ',midname) as name FROM  project_team left join employee on project_team.eid = employee.eid ");
		while($row= mysqli_fetch_assoc($query)){ ?>
			<tr>
				<td><?php echo ucwords($row['name']) ?></td>
				<td>
					<?php $query2 = mysqli_query($conn2,"SELECT *,Concat(firstname,', ',lastname,' ',midname) as name FROM  team_member left join employee on team_member.eid = employee.eid where tid = '".$row['tid']."' ");
