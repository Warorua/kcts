		...
		<div class="panel-body">
			<div class="col-md-4">
				...
				<div class="row">
					...
					<div class="col-sm-8 text-left"><label class="control-label" style="font-size:18px !important"><?php echo $row['settings_name'] ?></label></div>
				</div>
				<div class="row">
					<div class="col-sm-4 text-right"><label class="control-label">Start Date:</label></div>
					<div class="col-sm-8 text-left"><label class="control-label"><?php echo date("F d, Y", strtotime($row['settings_release_date'])) ?></label></div>
				</div>
				<div class="row">
				...
				<div class="row">
					<div class="col-sm-4 text-right"><label class="control-label">Project Manager:</label></div>
				...
				<div class="row">
					...
					<div class="col-sm-8 text-left"><label class="control-label"><?php if ($row['stats'] == '1') {
																						echo 'On going';
																					} elseif ($row['stats'] == '2') {
																						echo 'Finished';
																					} elseif ($row['stats'] == '3') {
				...
				<form action="../forms/add_forms.php?action=progress" method="POST" enctype="multipart/form-data">
					<div class="row">
						...
						<div class="col-sm-8">
							...
							<?php
							$div = mysqli_query($conn2, "SELECT * FROM project_partition natural join project_division where project_id = '$id' order by division ");
			...
			<div class="col-md-8">
				<div class="row">
					...
					<div class="col-sm-12" id="progress2">
						...
						$prog = $conn2->query("SELECT * FROM project_partition natural join project_division where project_id = '$id' ");
						while ($progress = $prog->fetch_assoc()) {

							$name = $progress['division'];
							$pid = $progress['pp_id'];
							$i = 1;
							$prog3 = $conn2->query("SELECT SUM(progress) as total_prog FROM project_progress  where pp_id = '$pid' ");
						...
						$prog2 = $conn2->query("SELECT SUM(progress) as total FROM project_progress natural join project_partition where project_id = '$id' ");
				...
				<table class="table table-bordered">
					...
					<tbody>
						<?php
						include '../includes/db.php';
						$query = mysqli_query($conn2, "SELECT * FROM project_partition natural join project_progress left join project_division on project_partition.pd_id = project_division.pd_id where project_id = '$id' order by date_added DESC ");
						if (mysqli_num_rows($query) > 0) {
							while ($row2 = mysqli_fetch_assoc($query)) {
						?>
								<tr>
									<td>
										<center><img src="../images/<?php echo $row2['partition_img'] ?>" width="100px" height="100px"></center>
									</td>
									<td><?php echo ucwords($row2['division']) ?></td>
									<td><?php echo $row2['progress'] . '%'; ?></td>
									<td><?php echo date("F d,Y", strtotime($row2['date_added'])) ?></td>
									<td>
										<center><a href="#progess<?php echo $row2['prog_id'] ?>" data-toggle="modal"><i class="fa fa-pencil"> </i> Edit</a></center>
									</td>
								</tr>
